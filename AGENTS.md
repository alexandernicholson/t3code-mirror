...
- Web screenshots and browser checks use `test-t3-app`: prefer the collaborative preview when an Electron host is connected; otherwise use `vp run browser:install`, `vp run browser:doctor`, `vp run screenshots:web`, and `vp run test:web:browser`. `vp run dev` alone does not provide an Electron preview host. An explicit request for screenshots or browser testing authorizes the required browser work; do not ask again.

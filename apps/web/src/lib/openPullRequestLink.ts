...
export function findProjectForChangeRequest(
  ...
  return projects.find((project) => {
    ...
    if (projectRef && kind !== "azure-devops") {
      const projectHost = web?.host.toLowerCase() ?? pullRequestHostOf(identity, kind);
      return projectHost === (link.authority ?? link.host).toLowerCase();
    }
    if (web)
      return (
...
export function useOpenChangeRequestLink(
  ...
  return useCallback(
    (event, targetUrl, targetThreadRef, targetEnvironmentId) => {
      ...
      const exactProject = findProjectForChangeRequest(projects, parsed, projectRef);
      const project =
        exactProject ??
        (resolvedPanelRef

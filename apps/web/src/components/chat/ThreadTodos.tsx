...
export const ThreadTodos = memo(function ThreadTodos({
  ...
  useEffect(() => {
    const toggle = (event: Event) => {
      const target = (event as CustomEvent<{ environmentId: string; threadId: string }>).detail;
      if (target?.environmentId === environmentId && target.threadId === threadId)
        setExpanded((value) => !value);
    };
    window.addEventListener("t3code:toggle-todos", toggle);
    return () => window.removeEventListener("t3code:toggle-todos", toggle);
  ...
  return (
    <section
      aria-label="TODOs"
      className="mb-2 rounded-xl border border-border bg-card text-sm shadow-sm"
    >
      <button
        type="button"
      ...
      {expanded && (
        <div className="border-t border-border p-3">
          <ul className="max-h-64 space-y-1 overflow-y-auto" aria-label="TODO list">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Edits are saved here and shared with the agent on your next message.
          </p>
          {error && (

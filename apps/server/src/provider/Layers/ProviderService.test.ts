...
describe("agent browser access", () => {
  ...
  const startSessionWith = (
    ...
    options?: {
      ...
      readonly managedMcpServers?: ManagedMcpServers;
      readonly onStarted?: (servers: ManagedMcpServers) => void;
    },
  ) =>
    Effect.gen(function* () {
      ...
      const providerLayer = makeProviderServiceLive({
        ...
        Layer.provide(
          ServerSettings.ServerSettingsService.layerTest({
            ...
            managedMcpServers: options?.managedMcpServers ?? {},
      ...
      yield* Effect.gen(function* () {
        ...
        if (options?.onStarted) {
          options.onStarted(ManagedMcp.readManagedMcpServers(threadId));
          yield* provider.stopSession({ threadId });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const issued = yield* startSessionWith(false, asThreadId("managed-mcp"), undefined, {
          managedMcpServers: {
            docs: shared,
          ...
          onStarted: (servers) => assert.deepEqual(servers, { docs: shared }),
        });
  ...
  it.effect("issues a credential with preview when agent browser access is on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["device", "preview", "pull-requests", "secrets"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("requests an MCP credential when the project overrides browser access to on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["preview", "pull-requests", "secrets"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
